...
A local-first web app that turns photos of paper receipts into structured, persisted data. The user uploads an image, a durable workflow extracts the text and parses it into a fixed contract, and the result is stored alongside the receipt image.
...
**Receipt image**: The JPEG stored in object storage for a receipt. The browser downscales and re-encodes the selected photo into it before upload, so it is a derivative, not the user's original file. _Avoid_: Source image, raw image, original photo.

**Object key**: The unique identifier object storage uses to locate a receipt image inside a bucket. Generated by the server and stored on the receipt row. _Avoid_: File path, filename, S3 path.

**Upload**: The act of moving the receipt image from the browser into object storage. The browser performs this directly using a presigned URL issued by the server. _Avoid_: Import, ingest.

**Presigned URL**: A short-lived, server-signed URL that grants the browser permission to write one specific object key to object storage without exposing real credentials. _Avoid_: Upload token, signed link.

**Bucket notification**: An event object storage sends to a configured webhook after an object is created. The server uses this as the authoritative signal that an upload finished. _Avoid_: Storage webhook, SQS event, object event.
