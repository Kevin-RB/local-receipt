...
- **Data**: `postgres-data` and `minio-data` named volumes. No host ports for app/postgres/minio — the Cloudflare tunnel is the only ingress: one wildcard route `*.tribi.dev → http://localhost:80` hits the Coolify Traefik proxy (T15), which routes by domain to the app/minio services.
