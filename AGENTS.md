...
- **Secrets only in Coolify env vars** — credentials are generated by Coolify magic variables (`SERVICE_USER_MINIO`, `SERVICE_PASSWORD_64_POSTGRES` / `MINIO` / `MINIOWEBHOOK` / `BETTERAUTH`) and the repo holds no credentials. Required non-credential vars: `POSTGRES_USER`, `POSTGRES_DB`, `MINIO_BUCKET`, `MINIO_PUBLIC_ENDPOINT`, `BETTER_AUTH_URL`, `INVITE_CODE`, `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY`. Optional: `LM_STUDIO_URL`, `ORC_MODEL`, `PARSE_MODEL`, `MINIO_CORS_ORIGIN`, plus per-environment admin host-port overrides `POSTGRES_HOST_PORT` (default `55432`) and `MINIO_CONSOLE_HOST_PORT` (default `59001`) — prod and staging share one Docker host, so staging must override these to avoid host-port collisions. The S3 API port is intentionally not published (app and minio-init use the compose network; presigned traffic goes via Traefik).
...
- **Data**: `postgres-data` and `minio-data` named volumes. No publicly-exposed host ports — the app publishes none at all, and postgres/minio publish admin ports (the `55432`/`59001` family, per-environment overrides above) on the mini's interfaces for LAN/tailnet use only. The Cloudflare tunnel is the only public ingress: one wildcard route `*.tribi.dev → http://localhost:80` hits the Coolify Traefik proxy (T15), which routes by domain to the app/minio services. `coolbox.tribi.dev` (Coolify admin) and the other non-app hostnames sit behind a Cloudflare Access gate (T16).
