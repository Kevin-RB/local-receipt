...
- **Data**: `postgres-data` and `minio-data` named volumes. No publicly-exposed host ports — the app publishes none at all, and postgres/minio publish admin ports (the `55432`/`55400`/`59001` family, per-environment overrides above) on the mini's interfaces for LAN/tailnet use only. The Cloudflare tunnel is the only public ingress: one wildcard route `*.tribi.dev → http://localhost:80` hits the Coolify Traefik proxy (T15), which routes by domain to the app/minio services. `coolbox.tribi.dev` (Coolify admin) and the other non-app hostnames sit behind a Cloudflare Access gate (T16).
